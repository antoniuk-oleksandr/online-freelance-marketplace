<script lang="ts">
  import ServiceBottomLeftSideContentLayout from '@/pages/services/components/ServiceBottomLeftSideContent/ServiceBottomLeftSideContentLayout.svelte'
  ...
  import { handleShowMoreFreelancesButtonClick } from '@/pages/services/handlers'
...
{#if serviceProps.serviceResponse.status === 200}
  <ServiceBottomLeftSideContentLayout>
    ...
    <UserByIdReviewBlock
      ...
      showMoreReviewsButtonAction={() =>
        handleShowMoreFreelancesButtonClick(serviceProps.setServiceResponse)}
      showServices={false}
