...
type AuthService interface {
	...
	SignIn(signInBody body.SignInBody) (*model.SignResponse, *dto.SignInData, error)
	SignUp(user *body.SignUpBody) error
	SignOut(signOutBody body.SignOut) error
	GetUserSessionData(userId int64) (*dto.UserSessionData, error)
	CheckIfEmailIsAvailable(email string) (bool, error)
	SignInWithGoogle(signInBody *body.GoogleSignInBody) (*model.SignResponse, *dto.SignInData, error)
	SignUpWithGoogle(signUpBody *body.GoogleSignUpBody) (*model.SignResponse, *dto.SignInData, error)
	CreateSignInTokens(usernameOrEmail string) (*model.SignResponse, *dto.SignInData, error)
	CreateSignInTokensWithData(googleUserInfo *model.GoogleUserInfo, userId int64, signUpBody *body.GoogleSignUpBody) (*model.SignResponse, *dto.SignInData, error)
}
